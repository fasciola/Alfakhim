...
export default function RainOnGlass() {
  ...
  useEffect(() => {
    ...
    const vs = compile(glContext.VERTEX_SHADER, VERT_SRC);
    const fs = compile(glContext.FRAGMENT_SHADER, FRAG_SRC);
    if (!vs || !fs) return;
    ...
    if (!glContext.getProgramParameter(prog, glContext.LINK_STATUS)) {
      console.error('Link error:', glContext.getProgramInfoLog(prog));
      return;
    }
    ...
    const uSheen = glContext.getUniformLocation(prog, 'u_sheenIntensity');
    ...
    function render(now: number) {
      ...
      glContext.uniform1f(uFlow, 0.4);
      glContext.uniform1f(uSheen, 1.0);
